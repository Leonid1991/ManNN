...
import numpy as np
import matplotlib
matplotlib.use('WXAgg', force=True) # change matplotlib backend, to remove annoying warning  
...
x_test[:,:-2] -= mean[:-2]
x_test[:,:-2] /= std[:-2]
# 2 Categories (yes-1 /no-0), such we use binary, there is no need for categorization
# # Creation NN
...
model.add(Dense(1, activation="sigmoid", kernel_initializer="normal"))                # yes/no
model.compile(loss="binary_crossentropy", optimizer="adam", metrics=["accuracy"]) # some random optimization routine
# Training procedure
history = model.fit(x_train, y_train, batch_size=30, epochs=nepoch, validation_split = valid_proportion, verbose=1)
# Model evaluation
